			...
			parallel{
				stage('PHPUnit test'){
					steps{
						...
						sh label: '', script: 'phpunit8 --log-junit phpunit-result.xml -c phpunit.xml'
					}
				}
				...
				stage('SonarQube test'){
					steps{
						...
						sh '/opt/sonar_scanner/bin/sonar-scanner -Dsonar.projectKey=sonarqube -Dsonar.projectName=SonarQube -Dsonar.projectVersion=1.0 -Dsonar.sources=/var/lib/jenkins/workspace/CICDPipeline/www -Dsonar.php.coverage.reportPaths=/var/lib/jenkins/workspace/CICDPipeline/phpunit-result.xml'
